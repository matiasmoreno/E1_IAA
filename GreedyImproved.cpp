			...
			if (cost[position[i]][j] != 0){
                if (production[j] > 0)
                {
                    if ((capacity[i] - production[j] - avg*p) > 0)
                    {
                        ...
                        {
                            value = (float(production[j]) - (float(actualPrize) + float(production[j]) - float(minPrize))) / ((float(cost[position[i]][j]) + float(cost[position[j]][0])));
                        }
                        else
                        {
                            value = float(production[j]) / ((float(cost[position[i]][j]) + float(cost[position[j]][0])));
                        }
                        if (maxValue < value)
                        {
                            maxValue = value;
                    ...
                    else
                    {
                        ...
                        {
                            value = (float(production[j]) - (float(actualPrize) + float(production[j]) - float(minPrize))) / float(cost[position[i]][j]);
                        }
                        else
                        {
